...
import * as fromNewsReducer from './news.reducer';
import { state } from '@angular/core';

describe('The news reducer', () => {
    const mockNewsList = [
        {
            'section': 'Briefing',
            'subsection': '',
    ...
    it('should load news, when LOAD_SECTION_NEWS dispatched', async() => {
        ...
        expect(result.newsList).toEqual(mockNewsList);
    });
    it('should update the filter, when FILTER_SUBSECTION is dispatched', async() => {
        const createAction = { type: fromNewsReducer.FILTER_SUBSECTION, payload: 'tech' };
        const result = news(fromNewsReducer.initialState, createAction);

        expect(result.filter).toBe('tech');
    });
    it('should return the list of news, when getNewsList() is called', async() => {
        const demoState = mockNewsList;
        const result = fromNewsReducer.getNewsList(demoState);

        expect(result).toBe(mockNewsList);
    });
    it('should return the filter value, when getFilter() is called', async() => {
        const demoState = {
          newsList: [
            { 'someData' : 'Value' }
          ],
        ...
        const result = fromNewsReducer.getFilter(demoState);
